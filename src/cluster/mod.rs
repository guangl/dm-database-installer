...
/// 集群部署入口（Plan 03 实现完整编排逻辑）。
pub async fn run(args: &crate::cli::ClusterDeployArgs) -> Result<()> {
    run_with_runners_impl(args).await
}

async fn run_with_runners_impl(args: &crate::cli::ClusterDeployArgs) -> Result<()> {
    use crate::config::cluster::load_cluster_config;
    use crate::cluster::ssh::SshSession;
    use std::sync::Arc;
    ...
    let mut runners: Vec<(crate::config::cluster::NodeConfig, Arc<dyn ssh::CommandRunner>)> =
        Vec::new();
    for node in &config.cluster.nodes {
        let session = SshSession::connect(&node.host, 22, &node.ssh.user, &node.ssh)
            .await
...
pub async fn run_with_runners(
    config: crate::config::cluster::ClusterConfig,
    runners: Vec<(crate::config::cluster::NodeConfig, std::sync::Arc<dyn ssh::CommandRunner>)>,
...
async fn run_preflight(
    runners: &[(crate::config::cluster::NodeConfig, std::sync::Arc<dyn ssh::CommandRunner>)],
) -> Result<()> {

...
fn save_runtime_state(dir: &Path) {
    ...
    let _ = std::fs::write(
        dir.join("firewall"),
        if fw_active { "active" } else { "inactive" },
    );

    if let Ok(o) = Command::new("timedatectl")
...
fn restore_derived(dir: &Path) {
    ...
    if Path::new(thp).exists()
        && let Ok(val) = std::fs::read_to_string(dir.join("thp"))
    {
        let _ = std::fs::write(thp, val.trim());
    }
    ...
    if let Ok(mode) = std::fs::read_to_string(dir.join("selinux_mode")) {
        match mode.trim() {
            "Enforcing" => {
                let _ = Command::new("setenforce").arg("1").status();
            }
            "Permissive" => {
...
impl Drop for StandaloneRollback {
    fn drop(&mut self) {
        ...
        // 清除 checkpoint，防止下次运行误用回滚后已失效的状态
        let _ = super::checkpoint::Checkpoint::remove();
    }
}

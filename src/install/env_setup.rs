...
async fn setup_dmdba_user(runner: &dyn CommandRunner) -> Result<()> {
    ...
    exec_r(runner,
        ...
        "创建用户 dmdba 失败").await?;
    exec_r(runner, "echo 'dmdba:dmdba' | chpasswd", "设置 dmdba 密码失败").await?;
...
async fn disable_selinux(runner: &dyn CommandRunner) -> Result<()> {
    ...
    let label = if mode.trim().is_empty() || mode.trim() == "Disabled" { "已禁用" } else { mode.trim() };
...
async fn set_timezone(runner: &dyn CommandRunner) -> Result<()> {
    exec_r(runner, "timedatectl set-timezone Asia/Shanghai 2>/dev/null || true", "设置时区失败").await?;
    let val = read_str(runner, "timedatectl show --property=Timezone 2>/dev/null || echo ''").await;
...
async fn set_locale(runner: &dyn CommandRunner) -> Result<()> {
    exec_r(runner,
        ...
        "设置字符集失败").await?;
    let val = read_str(runner, "grep 'LANG=zh_CN.UTF-8' /etc/profile 2>/dev/null || echo ''").await;
...
async fn configure_sysctl(runner: &dyn CommandRunner) -> Result<()> {
    ...
    let has_shmall = read_str(runner,
        "grep -c '^kernel.shmall' /etc/sysctl.conf 2>/dev/null || echo 0").await;
    if has_shmall.trim() == "0" || has_shmall.trim().is_empty() {
        ...
        exec_r(runner,
            ...
            "配置 kernel.shmall 失败").await?;
        exec_r(runner,
            &format!("echo 'kernel.shmmax={shmmax}' >> /etc/sysctl.conf"),
            "配置 kernel.shmmax 失败").await?;
    }
    exec_r(runner, "sysctl -p >/dev/null 2>&1 || true", "应用 sysctl 参数失败").await?;
...
async fn compute_shm(runner: &dyn CommandRunner) -> (u64, u64) {
    let meminfo = read_str(runner, "grep '^MemTotal:' /proc/meminfo 2>/dev/null || echo ''").await;

...
- **D-04:** 三个构建目标：
  ...
  - `x86_64-pc-windows-gnu` — PLAT-03 (Windows 控制机，SSH 到 Linux 节点)
- **D-05:** 使用 `cross` 工具链进行交叉编译（`cargo install cross --git https://github.com/cross-rs/cross`），Docker-based 无需配置额外 sysroot。`cargo-dist` 的 CI 环境可直接调用 cross。
